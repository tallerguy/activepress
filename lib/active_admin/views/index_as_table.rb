    ...
    class IndexAsTable < ActiveAdmin::Component
      ...
      def build(page_presenter, collection)
        table_options = {
          ...
          :sortable => true,
          :class => "index_table zebra-striped",
        ...
        def default_actions(options = {})
          ...
          column options[:name] do |resource|
            ...
            if controller.action_methods.include?('show')
              links += link_to I18n.t('active_admin.view'), resource_path(resource), :class => "member_link btn small"
            end
            if controller.action_methods.include?('edit')
              links += link_to I18n.t('active_admin.edit'), edit_resource_path(resource), :class => "member_link btn small"
            end
            if controller.action_methods.include?('destroy')
              links += link_to I18n.t('active_admin.delete'), resource_path(resource), :method => :delete, :confirm => I18n.t('active_admin.delete_confirmation'), :class => "member_link btn small"
            end
            links
          end
        end
